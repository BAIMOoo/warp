...
use crate::{
    ...
    settings::{AISettings, AppLocalizationSettings, PrivacySettings},
...
fn localized_secret_display_mode_dropdown_items(
    app: &AppContext,
) -> Vec<DropdownItem<PrivacyPageAction>> {
...
impl PrivacyPageView {
    ...
    pub fn new(ctx: &mut ViewContext<PrivacyPageView>) -> Self {
        ...
        let add_regex_modal_view = ctx.add_typed_action_view(|ctx| {
            Modal::new(
                Some(localized_settings_text("Add regex pattern", ctx).to_string()),
                add_regex_body,
                ctx,
            )
        ...
        let secret_display_dropdown = ctx.add_typed_action_view(|ctx| {
            ...
            dropdown.set_items(localized_secret_display_mode_dropdown_items(ctx), ctx);
            dropdown
        });
        ...
        privacy_page_view.update_cached_localized_controls(ctx);
        privacy_page_view
    }
    ...
    fn update_cached_localized_controls(&mut self, ctx: &mut ViewContext<Self>) {
        self.add_regex_modal_state.update_title(
            Some(localized_settings_text("Add regex pattern", ctx).to_string()),
            ctx,
        );

        let current_mode = get_effective_secret_display_mode(SafeModeSettings::as_ref(ctx));
        let items = localized_secret_display_mode_dropdown_items(ctx);
        self.secret_redaction_display_dropdown
            .update(ctx, |dropdown, ctx| {
...
impl SecretRedactionWidget {
    ...
    fn render_tab_bar(
        ...
        let personal_tab = self.render_tab(
            localized_settings_text("Personal", app).to_string(),
            personal_count,
        ...
        let enterprise_tab = self.render_tab(
            localized_settings_text("Enterprise", app).to_string(),
            enterprise_count,
        ...
        if is_enterprise_tab_active {
            ...
            row.add_child(
                self.render_info(
                    localized_settings_text("Enterprise secret redaction cannot be modified.", app)
                        .to_string(),
                    appearance,
                ),
            );
                ...
                Container::new(
                    Flex::row()
                        ...
                        .with_child(self.render_section_title(
                            localized_settings_text("Recommended", app).to_string(),
                            appearance,
                        ))
                        .with_child(
                            Container::new(
                                ui_builder
                                    ...
                                    .with_text_and_icon_label(Self::add_button(
                                        localized_settings_text("Add all", app),
                                        appearance,
                                    ))
...
impl SettingsWidget for SecretRedactionWidget {
    ...
    fn render(
            ...
            Flex::row()
                .with_child(
                    Shrinkable::new(
                        ...
                        render_sub_header(
                            appearance,
                            localized_settings_text(SAFE_MODE_TITLE, app),
                ...
                .with_child(
                    Container::new({
                        if is_enterprise_enabled {
                            self.render_info(
                                localized_settings_text("Enabled by your organization.", app)
                                    .to_string(),
                                appearance,
                            )
                        } else {
        ...
        let mut column = Flex::column()
            ...
            .with_child(
                ui_builder
                    .paragraph(localized_settings_text(*SAFE_MODE_DESCRIPTION, app))
                    .with_style(UiComponentStyles {
                        font_color: Some(description_text_color),
        ...
        if *safe_mode_settings.safe_mode_enabled {
            ...
            let label_with_icon = super::settings_page::render_dropdown_item_label(
                localized_settings_text("Secret visual redaction mode", app).to_string(),
                None,
                ...
                Flex::row()
                            ...
                            Flex::column()
                                .with_child(
                                    self.render_section_title(
                                        localized_settings_text(USER_SECRET_REGEX_TITLE, app)
                                            .to_string(),
                                        appearance,
                                    ),
                                )
                                .with_child(
                                    self.render_description(
                                        localized_settings_text(USER_SECRET_REGEX_DESCRIPTION, app)
                                            .to_owned(),
                                        appearance,
                                        if privacy_settings.user_secret_regex_list.iter().count()
                                            > 0
                                        {
                                            10.
                                        } else {
                    ...
                    .with_child(
                        ui_builder
                            ...
                            .with_text_and_icon_label(Self::add_button(
                                localized_settings_text("Add regex", app),
                                appearance,
                            ))
                            .with_style(Self::add_button_style())
                            .build()
...
impl SettingsWidget for AppAnalyticsWidget {
    ...
    fn render(
        ...
        let zdr_label_component = if self.should_show_zdr_badge(app) {
            Flex::row()
                ...
                .with_child(render_body_item_label::<PrivacyPageAction>(
                    localized_settings_text(TELEMETRY_TITLE, app).into(),
                    None,
                    None,
        ...
        if !is_on_paid_plan {
            column.add_child(
                ui_builder
                    .paragraph(localized_settings_text(TELEMETRY_FREE_TIER_NOTE, app))
                    .with_style(UiComponentStyles {
                        font_color: Some(description_text_color),
                        margin: Some(
        ...
        column.add_child(
            Align::new(
                ui_builder
                    .link(
                        localized_settings_text("Read more about Warp's use of data", app).into(),
    ...
    fn render(
        ...
        Flex::column()
            .with_child(render_body_item::<PrivacyPageAction>(
                localized_settings_text("Send crash reports", app).into(),
                None,
...
impl SettingsWidget for CloudConversationStorageWidget {
    ...
    fn render(
        ...
        Flex::column()
            .with_child(render_body_item::<PrivacyPageAction>(
                localized_settings_text("Store AI conversations in the cloud", app).into(),
                None,
                LocalOnlyIconState::Hidden,
                toggle_state,
                appearance,
...
impl SettingsWidget for NetworkLogWidget {
    ...
    fn render(
        ...
        Flex::column()
            .with_child(render_body_item::<PrivacyPageAction>(
                localized_settings_text("Network log console", app).into(),
                None,
            ...
            .with_child(
                Align::new(
                    ui_builder
                        .link(
                            localized_settings_text("View network logging", app).to_owned(),
                            None,
                            Some(Box::new(|ctx| {
...
impl SettingsWidget for DataManagementWidget {
    ...
    fn render(
        ...
        Flex::column()
            .with_child(render_body_item::<PrivacyPageAction>(
                localized_settings_text(DATA_MANAGEMENT_TITLE, app).into(),
                None,
                // Not rendering a setting, so no need to show local only icon state.
                LocalOnlyIconState::Hidden,
                ToggleState::Enabled,
            ...
            .with_child(
                ui_builder
                    .paragraph(localized_settings_text(DATA_MANAGEMENT_DESCRIPTION, app))
                    .with_style(UiComponentStyles {
                        font_color: Some(
                            appearance
                                .theme()
            ...
            .with_child(
                Align::new(
                    appearance
                        .ui_builder()
                        .link(
                            localized_settings_text(DATA_MANAGEMENT_LINK_TEXT, app).into(),
                            None,
                            Some(Box::new(|ctx| {
                                ctx.dispatch_typed_action(
                                    PrivacyPageAction::OpenDataManagementWebpage,
...
impl SettingsWidget for PrivacyPolicyWidget {
    ...
    fn render(
        ...
        Flex::column()
            .with_child(render_body_item::<PrivacyPageAction>(
                localized_settings_text(PRIVACY_POLICY_TITLE, app).into(),
                None,
                // Not rendering a setting, so no need to show local only icon state.
                LocalOnlyIconState::Hidden,
                ToggleState::Enabled,
            ...
            .with_child(
                Align::new(
                    appearance
                        .ui_builder()
                        .link(
                            localized_settings_text(PRIVACY_POLICY_LINK_TEXT, app).into(),
